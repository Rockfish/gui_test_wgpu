...
pub fn build(b: *std.Build) void {
    ...
    switch (target.result.os.tag) {
        ...
        .linux => {
            glfw.addSystemIncludePath(b.path(system_sdk.path("linux/include").getPath(b) ));
            glfw.addSystemIncludePath(b.path(system_sdk.path("linux/include/wayland").getPath(b) ));
            glfw.addIncludePath(b.path(src_dir ++ "wayland" ));

            if (target.result.cpu.arch.isX86()) {
                glfw.addLibraryPath(b.path(system_sdk.path("linux/lib/x86_64-linux-gnu").getPath(b) ));
            } else {
                glfw.addLibraryPath(b.path(system_sdk.path("linux/lib/aarch64-linux-gnu").getPath(b) ));
            }
            glfw.addCSourceFiles(.{
                .files = &.{
                    src_dir ++ "platform.c",
    ...
    switch (target.result.os.tag) {
        .linux => {
            tests.addSystemIncludePath(b.path(system_sdk.path("linux/include").getPath(b)));
            if (options.enable_wayland) {
                glfw.addSystemIncludePath(b.path(system_sdk.path("linux/include/wayland").getPath(b)));
            }
        },
