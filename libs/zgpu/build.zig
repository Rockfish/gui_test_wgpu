...
pub fn addLibraryPathsTo(compile_step: *std.Build.Step.Compile) void {
    ...
    switch (target.os.tag) {
        .windows => {
            const path = b.dependency("dawn_x86_64_windows_gnu", .{}).path("");
            compile_step.addLibraryPath(path);
        },
        .linux => {
            const path = b.dependency(if (target.cpu.arch.isX86()) "dawn_x86_64_linux_gnu" else "dawn_aarch64_linux_gnu", .{}).path("");
            compile_step.addLibraryPath(path);
        },
        .macos => {
            const path = b.dependency(if (target.cpu.arch.isX86()) "dawn_x86_64_macos" else "dawn_aarch64_macos", .{}).path("");
            compile_step.addLibraryPath(path);
        },
        else => {},
    }
